...
class ThresholdScheduler:
    ...
    def schedule_future_approval(self, agent_id: str, set_id: int, approval_time: datetime) -> None:
        """Schedule an agent for future approval when threshold decays to their score"""
        task_key = f"{agent_id}_{set_id}"
        ...
        if delay_seconds <= 0:
            ...
            asyncio.create_task(self._approve_agent_now(agent_id, set_id))
            return
        
        # Schedule future approval
        task = asyncio.create_task(self._approve_agent_delayed(agent_id, set_id, delay_seconds))
        self._scheduled_tasks[task_key] = task
        self._pending_approvals.add(task_key)
        
        logger.info(f"Scheduled approval for {agent_id} on set {set_id} in {delay_seconds:.0f} seconds")
    
    async def _approve_agent_delayed(self, agent_id: str, set_id: int, delay_seconds: float) -> None:
        """Wait for delay then approve the agent"""
        task_key = f"{agent_id}_{set_id}"
        
        try:
            await asyncio.sleep(delay_seconds)
            await self._approve_agent_now(agent_id, set_id)
            logger.info(f"Successfully approved {agent_id} for set {set_id} after threshold decay")
        except asyncio.CancelledError:
            logger.info(f"Cancelled scheduled approval for {agent_id} on set {set_id}")
        except Exception as e:
            logger.error(f"Error in scheduled approval for {agent_id} on set {set_id}: {e}")
        finally:
            # Clean up
            self._scheduled_tasks.pop(task_key, None)
            self._pending_approvals.discard(task_key)
    
    async def _approve_agent_now(self, agent_id: str, set_id: int) -> None:
        ...
        try:
            ...
            await approve_agent_version(str(agent_id), set_id, None)
            ...
            logger.info(f"Agent {agent_id} approved and added to top agents history for set {set_id}")
            
        except Exception as e:
            logger.error(f"Failed to approve agent {agent_id} for set {set_id}: {e}")
    
    def cancel_scheduled_approval(self, agent_id: str, set_id: int) -> bool:
            ...
            async with get_transaction() as conn:
                ...
                for row in unapproved_agents:
                    ...
                    if result['action'] == 'approve_now':
                        ...
                        self.schedule_future_approval(agent_id, max_set_id, result['future_approval_time'])
                        recovery_count += 1
