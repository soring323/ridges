import asyncpg
from datetime import datetime

from utils.database import db_operation
# from api.src.backend.entities import ProviderStatistics

# @db_operation
# async def get_inference_provider_statistics(conn: asyncpg.Connection, start_time: datetime, end_time: datetime) -> list[ProviderStatistics]:
#     provider_stats = await conn.fetch(f"""
#         SELECT 
#             provider,
#             AVG(EXTRACT(EPOCH FROM (response_sent_at - request_received_at))) as avg_time_taken,
#             PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY EXTRACT(EPOCH FROM (response_sent_at - request_received_at))) as median_time_taken,
#             PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY EXTRACT(EPOCH FROM (response_sent_at - request_received_at))) as p95_time_taken,
#             MAX(EXTRACT(EPOCH FROM (response_sent_at - request_received_at))) as max_time_taken,
#             MIN(EXTRACT(EPOCH FROM (response_sent_at - request_received_at))) as min_time_taken,
#             COUNT(*) as total_requests,
#             COUNT(CASE WHEN status_code = 200 THEN 1 END) as successful_requests,
#             COUNT(CASE WHEN status_code != 200 THEN 1 END) as failed_requests,
#             CASE 
#                 WHEN COUNT(*) > 0 THEN 
#                     (COUNT(CASE WHEN status_code != 200 THEN 1 END)::float / COUNT(*)::float) * 100
#                 ELSE 0 
#             END as error_rate,
#             SUM(COALESCE(num_input_tokens, 0) + COALESCE(num_output_tokens, 0)) as total_tokens
#         FROM inferences 
#         WHERE request_received_at >= $1
#         AND request_received_at <= $2
#         AND response_sent_at IS NOT NULL
#         AND provider IS NOT NULL
#         GROUP BY provider
#         ORDER BY provider
#     """, start_time, end_time)
    
#     return [ProviderStatistics(**stat) for stat in provider_stats]
