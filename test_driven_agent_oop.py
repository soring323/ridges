...
def _generate_git_patch_helper(generated_test_files: List[str]) -> str:
    """
    Generate a clean unified diff (staged changes only) that tools like `patch`
    or `git apply` can consume.
    
    This implementation is adapted from supermount.py to properly handle:
    - Polyglot dataset: All test files are generated by agent (should be included in patch)
    - SWE-bench: Mix of existing test files (include) and agent-generated test files (exclude)
    
    Args:
        generated_test_files: List of test file paths that were generated by the agent
                            during the workflow and should be excluded from the patch
    """
    try:
        ...
        # This is critical for SWE-bench where we want to include existing test files
        # but exclude test files generated by the agent during debugging
        try:
            for _p in generated_test_files:
                # Store as relative paths similar to git ls-files output
                exclude.add(os.path.relpath(_p) if os.path.isabs(_p) else _p)
        except Exception as e:
            print(f"[GIT] Warning: Error processing generated_test_files: {e}")
        ...
        if diff.stderr:
            print(f"[GIT] git diff (stderr): {diff.stderr.strip()}")

        patch_text = diff.stdout or ""
        return patch_text
    except Exception as e:
        print(f"[GIT] Error generating git patch: {e}")
        import traceback
        traceback.print_exc()
    ...
    def generate_tests(self, problem: str, solution: Dict[str, str]) -> Dict[str, str]:
        ...
        try:
            ...
            if files:
                ...
                # Only rename if the file already exists in the repo (to avoid overwriting)
                renamed_files = {}
                for filename, content in files.items():
                    # Check if this file already exists
                    if os.path.exists(filename):
                        # File exists - rename to avoid overwriting
                        new_name = f'test_{filename}' if not filename.startswith('test_') else f'generated_{filename}'
                        # Ensure the new name doesn't exist either
                        counter = 1
                        while os.path.exists(new_name):
                            base, ext = os.path.splitext(filename)
                            new_name = f'test_{base}_{counter}{ext}'
                            counter += 1
                        renamed_files[new_name] = content
                        print(f"[TEST_GEN] ⚠️ Renamed '{filename}' to '{new_name}' (file already exists in repo)")
                    else:
                        # File doesn't exist - keep the original name
...
class TestDrivenAgent:
    ...
    def __init__(
        ...
        self.generated_test_files = []  # Track test files generated by agent
    ...
    def solve_create(self, problem: str, timeout: int) -> str:
        ...
        if not existing_tests:
            ...
            created_files = self.file_manager.write_files(test_files)
            # Track generated test files for exclusion from patch (for polyglot mode)
            # Note: In polyglot, all tests are generated, but for consistency we track them
            for file_path in created_files:
                self.generated_test_files.append(file_path)
            print(f"[STEP 1] Created {len(created_files)} test files (tracked for patch exclusion)")
        else:
    ...
    def solve_fix(self, problem: str, timeout: int) -> str:
        ...
        if test_file:
            created_files = self.file_manager.write_files(test_file)
            # Track generated reproduction tests for exclusion from patch (for SWE-bench)
            # This ensures agent-generated test files are excluded, while existing repo tests are included
            for file_path in created_files:
                self.generated_test_files.append(file_path)
            print(f"[STEP 2] Created reproduction test (tracked for patch exclusion)")
    ...
    def _generate_patch(self, files: Dict[str, str]) -> str:
        """Generate git diff patch, excluding agent-generated test files."""
        return _generate_git_patch_helper(self.generated_test_files)
