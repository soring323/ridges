...
class EnhancedTOT:
    ...
    class Action:
        def __init__(self, next_thought: str, next_tool_name: str, next_tool_args: dict, observation: list|tuple|str,
                     is_error:bool=False, raw_response:str=None, total_attempts:int=0, 
                     inference_error_counter:dict=None, request_data:list=None, score:float=0.0):
            ...
            self.score=score  # Evaluation score for this action
    
    class Node:
        """Tree node representing a state in the search tree"""
        def __init__(self, action: 'EnhancedTOT.Action', parent: 'EnhancedTOT.Node'=None):
            self.action = action
            self.parent = parent
            self.children: list['EnhancedTOT.Node'] = []
            self.depth = 0 if parent is None else parent.depth + 1
            self.is_explored = False
            self.is_terminal = False  # True if this path succeeded or permanently failed
            
        def get_path(self) -> list['EnhancedTOT.Action']:
    ...
    def __init__(self, latest_observations_to_keep=5, max_branches=3, backtrack_threshold=2):
        """Initialize Tree of Thought
        
        Args:
            latest_observations_to_keep: Number of recent observations to keep in context
            max_branches: Maximum number of candidate branches to generate per step
            backtrack_threshold: Number of consecutive errors before backtracking
        """
        self.root = self.Node(action=None, parent=None)
        self.current_node = self.root
        self.latest_observations_to_keep = latest_observations_to_keep
        self.max_branches = max_branches
        self.backtrack_threshold = backtrack_threshold
        self.exploration_history = []  # Track which nodes we've explored
        self.consecutive_errors = 0
        
    def add_action(self, action: 'EnhancedTOT.Action', as_branch: bool = False) -> bool:
        """Add action to the current node or as a new branch
        
        Args:
            action: The action to add
            as_branch: If True, creates a sibling branch instead of extending current path
        """
        if action.is_error:
        ...
        new_node = self.Node(action=action, parent=self.current_node if not as_branch else self.current_node.parent)
        
        if as_branch and self.current_node.parent:
        ...
        self.exploration_history.append(new_node)
        return True
    
    def should_backtrack(self) -> bool:
        """Determine if we should backtrack to explore other branches"""
        return self.consecutive_errors >= self.backtrack_threshold
    
    def backtrack(self) -> bool:
        """Backtrack to the nearest unexplored branch
        
        Returns:
            True if backtracking was successful, False if no alternatives exist
        """
        logger.info("[TOT] Attempting to backtrack...")
        
        # Try to find an unexplored sibling first
        if self.current_node.parent:
            siblings = self.current_node.parent.children
            unexplored_siblings = [n for n in siblings if not n.is_explored and n != self.current_node]
            
            if unexplored_siblings:
                # Choose the best scored unexplored sibling
                best_sibling = max(unexplored_siblings, key=lambda n: n.action.score if n.action else 0)
                self.current_node = best_sibling
                self.consecutive_errors = 0
                logger.info(f"[TOT] Backtracked to sibling at depth {self.current_node.depth}")
                return True
        
        # Otherwise, go up the tree to find an ancestor with unexplored children
        node = self.current_node.parent
        while node is not None:
            unexplored_children = [n for n in node.children if not n.is_explored]
            if unexplored_children:
                best_child = max(unexplored_children, key=lambda n: n.action.score if n.action else 0)
                self.current_node = best_child
                self.consecutive_errors = 0
                logger.info(f"[TOT] Backtracked to ancestor's child at depth {self.current_node.depth}")
                return True
            node = node.parent
        
        logger.info("[TOT] No unexplored branches found, cannot backtrack")
        return False
    
    def get_current_path(self) -> list['EnhancedTOT.Action']:
        """Get the current path from root to current node"""
        return self.current_node.get_path()
    
    def is_thought_repeated(self)->bool:
        """Check if the last action repeats a recent action"""
    ...
    def mark_current_terminal(self, success: bool = True):
        """Mark current node as terminal (either success or permanent failure)"""
        self.current_node.is_terminal = True
        self.current_node.is_explored = True
    
    def evaluate_action(self, action: 'EnhancedTOT.Action') -> float:
        """Evaluate an action and assign it a score based on various heuristics
        ...
        # Reward successful tool executions
        if action.next_tool_name and not action.is_error:
            score += 0.1
            
        # Reward progress indicators in observations
        if action.observation and isinstance(action.observation, str):
            obs_lower = action.observation.lower()
            
            # Positive signals
            if any(word in obs_lower for word in ['success', 'passed', 'found', 'fixed', 'completed']):
                score += 0.15
            
            # Negative signals
            if any(word in obs_lower for word in ['error', 'failed', 'not found', 'exception']):
                score -= 0.15
        
        # Reward finish actions that succeeded
        if action.next_tool_name == 'finish' and not action.is_error:
            score += 0.3
            
        # Clamp score between 0 and 1
        return max(0.0, min(1.0, score))
    
    def should_explore_branches(self) -> bool:
        """Determine if we should generate multiple candidate branches at this step
        
        Returns True when:
        - We're at a critical decision point (error recovery, multiple options)
        - We haven't exceeded our branching budget
        """
        # Don't branch if we're deep in the tree already
        if self.current_node.depth > 20:
            return False
            
        # Branch when we've just recovered from an error
        path = self.get_current_path()
        if len(path) >= 2 and path[-1].is_error:
            return True
            
        # Branch occasionally to explore alternatives
        return self.current_node.depth % 5 == 0 and len(self.current_node.children) == 0
    
    def to_str(self):
        """Convert current path to message format for LLM"""
        messages = []
        thoughts = self.get_current_path()
        
        for i, thought in enumerate(thoughts):
            if thought.is_deleted:
                continue
            if i < len(thoughts) - self.latest_observations_to_keep:
                assistant_str = (
                ...
                if thought.is_error is None or i == len(thoughts) - 1:
                    ...
                    if thoughts[-1].is_error == None and thought.is_error != None:
                        assistant_str = (
                            f"next_thought:{thought.next_thought}\n"
                            f"next_tool_name:{thought.next_tool_name}\n"
                            f"next_tool_args:{thought.next_tool_args}")
    ...
    def export_to_csv(self, file_path: str = "./xray.csv"):
        """Export the entire exploration tree to CSV"""
        with open(file_path, "w") as f:
            writer = csv.writer(f)
            writer.writerow(["depth", "next_thought", "next_tool_name", "next_tool_args", "observation", 
                           "is_error", "raw_response", "total_attempts", "score", "is_deleted"])
            
            # Traverse the tree and export all nodes
            def traverse(node, writer):
                if node.action:
                    writer.writerow([
                        node.depth, node.action.next_thought, node.action.next_tool_name, 
                        node.action.next_tool_args, node.action.observation, node.action.is_error,
                        node.action.raw_response, node.action.total_attempts, node.action.score,
                        node.action.is_deleted
                    ])
                for child in node.children:
                    traverse(child, writer)
            
            traverse(self.root, writer)
...
def validate_solution(initial_solution: str, test_code: str) -> tuple[bool, str]:
    """
    Validate if the initial solution passes all tests without modification.
    Returns (success: bool, test_output: str) tuple.
    """
    try:
        ...
        for line in test_code_lines:
            stripped = line.strip()
            # Skip lines that are just filenames (e.g., "test_hangman.py")
            if stripped.endswith('.py') and ' ' not in stripped and len(stripped) < 50:
                logger.info(f"Skipping filename header in test code: {stripped}")
                continue
            cleaned_lines.append(line)
        
        cleaned_test_code = '\n'.join(cleaned_lines).strip()

        test_file_name = "validate_solution_test.py"
        with open(test_file_name, "w") as f:
            f.write(cleaned_test_code)

        result = subprocess.run(["python", test_file_name], capture_output=True, text=True, timeout=30) 
...
def process_create_task(input_dict):
    ...
    is_valid, test_output = validate_solution(initial_solution, test_cases)

    if is_valid:
        logger.info("[PROCESS_CREATE_TASK] ✓ Initial solution passed all tests") 
        extract_and_write_files(initial_solution)
...
def fix_task_solve_workflow(problem_statement: str, *, timeout: int, run_id_1: str, instance_id: str = "", \
    ...
    for step in range(n_max_steps):
        ...
        if time.time() - start_time > timeout:
            tot.add_action(EnhancedTOT.Action(next_thought="global timeout reached",next_tool_name="",next_tool_args={},observation="",is_error=True,inference_error_counter={},request_data=[]))
            break
        ...
        messages.extend(tot.to_str())

        messages.append({"role": "system", "content": STOP_INSTRUCTION})
    
        if tot.is_thought_repeated():
            logger.info(f"[TOT] Thought repeated, adding DO NOT REPEAT TOOL CALLS instruction")
            path = tot.get_current_path()
            if path:
                last_thought = path[-1]
        ...
        # Check if we should backtrack due to consecutive errors
        if tot.should_backtrack():
            logger.info("[TOT] Consecutive errors detected, attempting backtrack...")
            if tot.backtrack():
                logger.info("[TOT] Successfully backtracked, continuing from alternative path")
                continue
            else:
                logger.warning("[TOT] Backtrack failed, no alternative paths available")
    
        # Determine if we should explore multiple branches at this step
        explore_branches = tot.should_explore_branches() and tot.max_branches > 1
        num_candidates = tot.max_branches if explore_branches else 1
        
        if explore_branches:
            logger.info(f"[TOT] Exploring {num_candidates} candidate branches at depth {tot.current_node.depth}")
        
        candidates = []
        for candidate_idx in range(num_candidates):
            try:
                # Use higher temperature for branch exploration to get diverse candidates
                temperature = 0.3 if explore_branches else 0.0
                next_thought, next_tool_name, next_tool_args, raw_text, total_attempts, error_counter, _ = EnhancedNetwork.inference(
                    messages, model=GLM_MODEL_NAME, run_id=run_id, temperature=temperature
                )
                
                # Create candidate action (not yet added to tree)
                candidate_action = EnhancedTOT.Action(
                    next_thought=next_thought,
                    next_tool_name=next_tool_name,
                    next_tool_args=next_tool_args,
                    observation="",  # Will be filled after execution
                    is_error=False,
                ...
                if not explore_branches:
                    break  # Only need one candidate
                    
            except Exception as e:
                import traceback
                error_msg = f"\n\nERROR: {repr(e)} {traceback.format_exc()}"
                logger.error(f"Inference error: {error_msg}")
                if candidate_idx == 0:  # At least the first candidate failed
                    tot.add_action(EnhancedTOT.Action(
        ...
        if not candidates:
            logger.error("[TOT] No valid candidates generated")
            break
        
        # Select the best candidate to execute (or execute all if exploring)
        if explore_branches:
            logger.info(f"[TOT] Generated {len(candidates)} candidates, will execute and evaluate each")
        
        best_candidate_action = None
        best_score = -1.0
        
        for candidate_idx, candidate in enumerate(candidates):
            next_thought = candidate.next_thought
            next_tool_name = candidate.next_tool_name
            next_tool_args = candidate.next_tool_args
            raw_text = candidate.raw_response
            total_attempts = candidate.total_attempts
            error_counter = candidate.inference_error_counter
            
            if explore_branches:
                logger.info(f"[TOT] Evaluating candidate {candidate_idx + 1}/{len(candidates)}: {next_tool_name}")
            else:
            ...
            try:
                ...
                # Create action with observation
                action = EnhancedTOT.Action(
                    next_thought=next_thought,
                    next_tool_name=next_tool_name,
                    next_tool_args=next_tool_args,
                ...
                # Evaluate the action
                score = tot.evaluate_action(action)
                action.score = score
                logger.info(f"[TOT] Candidate {candidate_idx + 1} score: {score:.3f}")
                
                if explore_branches:
                    # Add as a branch (sibling) for evaluation
                    tot.add_action(action, as_branch=(candidate_idx > 0))
                    if score > best_score:
                        best_score = score
                        best_candidate_action = action
                else:
                    # Single candidate mode - just add it normally
                    tot.add_action(action)
                    best_candidate_action = action
        ...
        # If we explored branches, navigate to the best one
        if explore_branches and best_candidate_action:
            logger.info(f"[TOT] Selected best branch with score {best_score:.3f}")
            # Find the node with this action and make it current
            for child in tot.current_node.children:
                if child.action == best_candidate_action:
                    tot.current_node = child
                    break
        
        # Check termination conditions
        if not best_candidate_action:
            logger.warning("[TOT] No successful candidate, may need to backtrack")
            continue
            
        next_tool_name = best_candidate_action.next_tool_name
        
        if next_tool_name == "finish":
            logger.info('[CRITICAL] Workflow called finish operation')
            tot.mark_current_terminal(success=True)
            break
        ...
        tot.add_action(EnhancedTOT.Action(next_thought="global timeout reached",next_tool_name="",next_tool_args={},observation="",is_error=True))
